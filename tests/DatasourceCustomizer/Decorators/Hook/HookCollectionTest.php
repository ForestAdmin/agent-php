...
beforeEach(closure: function () {
    global $datasource, $datasourceDecorator, $collectionTransactions, $decoratedTransaction, $records, $aggregateResult;
...
test('schema should not be changed', function () {
    ...
    global $collectionTransactions, $decoratedTransaction;

    expect($collectionTransactions->getFields())->toEqual($decoratedTransaction->getFields());
});

test('when adding a before hook on a list it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'before', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['List' => $hook]);
...
test('when adding a before hook on a create it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'before', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['Create' => $hook]);
...
test('when adding a before hook on a update it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'before', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['Update' => $hook]);
    $data = ['description' => 'new transaction', 'amount' => 100];
...
test('when adding a before hook on a delete it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'before', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['Delete' => $hook]);
    $filter = new Filter(conditionTree: new ConditionTreeLeaf('id', Operators::EQUAL, 1));
...
test('when adding a before hook on a aggregate it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'before', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['Aggregate' => $hook]);
    $filter = new PaginatedFilter();
...
test('when adding a after hook on a list it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction, $records;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'after', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['List' => $hook]);
    $filter = new PaginatedFilter();
    $projection = new Projection();
...
test('when adding a after hook on a create it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction, $records;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
...
test('when adding a after hook on a update it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
...
test('when adding a after hook on a delete it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
...
test('when adding a after hook on a aggregate it should call the hook with valid parameters', function ($caller) {
    /** @var HookCollection $decoratedTransaction */
    global $decoratedTransaction, $aggregateResult;
    $mock = Mockery::mock(fn () => true);
    $hook = new Hooks();
    invokeProperty($hook, 'after', [$mock]);
    invokeProperty($decoratedTransaction, 'hooks', ['Aggregate' => $hook]);
